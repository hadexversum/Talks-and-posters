...
\title{\Huge Analysis of data from hydrogen-deuterium exchange \\ mass spectrometry experiments} 
...
The documentation and methodology is available on: 
\url{https://hadexversum.github.io/HaDeX/articles/overview.html}$\begin{array}{l}
        \begin{figure}[ht]
          \centering
          \scalebox{0.45}{
            \includegraphics{figure/qr_cods.png}
          }
        \end{figure}
      \end{array}
...
\begin{block}{Kinetics of the peptides}
  ...
  \justify \textbf{HaDeX} has a module for analyzing deuteration uptake change in time for given peptide in given time. For comparison, choosing multiple combinations of peptides and their states is possible for the plot. 
...
\begin{block}{Recommendations for performing an experiment}

  \vspace{0.5cm}
  \begin{columns}
    \begin{column}{0.55\textwidth}
    \justify \textbf{HaDeX} follows the recently published recommendations for performing, interpreting and reporting HDX-MS experiments. One aspect of that is summary of the data - for quality control purposes - as shown on the figure \ref{fig:sum}. The summary table can be downloaded in csv/pdf form, like all of the available data tables. 
    \end{column}
    \begin{column}{0.45\textwidth}
      \begin{figure}
        \includegraphics[width=0.5\columnwidth]{figure/hadex_summary_table.png}
...
\begin{block}{Report function}
  ...
  \begin{columns}
    \begin{column}{0.45\textwidth}
       \justify \textbf{HaDeX} provides report generation - with parameters chosen by user so is fully personalizable.  On figure \ref{fig:rep} are shown available elements of the report - that can be chosen by user and limited strickly for user needs.
    \end{column}
    \begin{column}{0.55\textwidth}  
      \begin{center}
        \begin{figure}
...
\begin{block}{Bibliography}
  ...
  \small
    ...
    \item [6] Masson GR, Burke JE, Ahn NG, Anand GS, Borchers C, Brier S, et al. Recommendations for performing, interpreting and reporting hydrogen deuterium exchange mass spectrometry (HDX-MS) experiments. Nat Methods. 2019 Jul;16(7):595–602. 
