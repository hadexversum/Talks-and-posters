...
\justify The Law of Propagation of Uncertainty, as described by equation \ref{eq:law_un} is applied to both \ref{eq:deut_lev} and \ref{eq:diff}. Uncertaintly calculated this way are compatibile with guidelines developed by International Organization for Standardization.
