...
\justify The \textbf{HaDeX} web server relies only on a single data format: DynamiX datafile. A user can customize each step of the analysis with his own input parameters.
...
\begin{figure}
\includegraphics[width=0.75\columnwidth]{figure/coverage.png}
...
\justify The levels of deuteration along with uncertainty intervals are shown on comparison plot~\ref{fig:comp_plots}. \textbf{HaDeX} provide both experimental and theroetical levels of deuteration in two variants: with relative or absolute values. 
...
\textbf{HaDeX} is available as a web server:
     \url{http://mslab-ibb.pl/shiny/HaDeX/}$\begin{array}{l}
        ...
        \centering
        \scalebox{0.025}{
          \includegraphics{figure/hadex_qr.eps}
        }
        \end{figure}
      \end{array}

as the R package:
\url{https://cran.r-project.org/web/packages/HaDeX/index.html} \\
and as a standalone software:
\url{https://sourceforge.net/projects/hadex/}
...
Equation \ref{eq:diff} is a function of two variables, so applying equation \ref{eq:law_un} we get uncertainty described by equation \ref{eq:diff_un}.
...
<<echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.align="center">>=
 ...
 w2 <- woods_plot(calc_dat = calc_dat,
            theoretical = FALSE) +
   labs(title = "Fraction exchanged between states \nin 25 min time") + 
   poster_theme +
   scale_y_continuous(limits = c(-0.5, 0.5), expand = c(0, 0))
...
\begin{column}{0.5\textwidth}  %%<--- here
    \begin{center}
     \begin{figure}
\includegraphics[width=0.7\columnwidth]{figure/report.png}
