...
\setlength{\columnheight}{100cm} % ------------------------------- od jakiej wysokości mają zaczynać się klocki, ale też miejsce dla tytułu autorów, afiliacji
...
\begin{block}{Main functionality}
  ...
  \begin{figure}
    \includegraphics[width=0.98\columnwidth]{static_figure/web_server.png}
...
\begin{block}{Kinetics of the peptides}
  ...
  \begin{figure}
    ...
    \includegraphics[width=0.98\columnwidth]{static_figure/kinetics.png}
...
\textbf{HaDeX} is available as a web server:
     \url{http://mslab-ibb.pl/shiny/HaDeX/}$\begin{array}{l}
        \begin{figure}[ht]
          \centering
          \scalebox{0.5}{
            \includegraphics{static_figure/hadex_qr.png}
          }
        \end{figure}
      \end{array}
...
\url{https://cran.r-project.org/web/packages/HaDeX/index.html}$\begin{array}{l}
        \begin{figure}[ht]
          \centering
          \scalebox{0.5}{
            \includegraphics{static_figure/cran_qr.png}
          }
        \end{figure}
      \end{array}
...
\url{https://sourceforge.net/projects/hadex/}$\begin{array}{l}
        \begin{figure}[ht]
          \centering
          \scalebox{0.5}{
            \includegraphics{static_figure/sourceforge_qr.png}
          }
        \end{figure}
      \end{array}
...
\begin{block}{Acknowledgements}
  ...
  \begin{figure}
   \includegraphics[width=0.95\columnwidth]{static_figure/listwa.png}
  \end{figure}
...
<<echo=FALSE,message=FALSE,fig.height=15,fig.width=14,results='asis'>>=
library(dplyr)
library(ggplot2)
tidy_hdx <- readRDS("tidy_hdx.RDS")
mutate(tidy_hdx, time_label = paste0(time, " [min]")) %>% 
  ggplot(aes(x = Start, y = value, xend = End, yend = value, color = variable)) +
  geom_segment(size = 1) +
  scale_color_manual("", values = c("blue", "red")) +
  scale_x_continuous("Position in sequence") +
  scale_y_continuous(expression(paste(Delta, " H/D fraction exchanged [%]"))) +
  theme_bw(base_size = 28) +
  theme(legend.position = "bottom") +
  facet_wrap(~ time_label, ncol = 2)
...
A multi-state variability test offers a statistical assessment of regional variability. An user can compare relative deuteration between multiple states in specific regions of the interest (ROI).

  \begin{figure}
    \includegraphics[width=0.98\columnwidth]{static_figure/example_curves.eps}
  \end{figure}
...
\begin{block}{Report function}
      ...
      \begin{center}
        \begin{figure}
          \includegraphics[width=0.9\columnwidth]{static_figure/report.png}
